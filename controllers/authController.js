...
exports.protect = catchAsync(async (req, res, next) => {
  let token;
  // 1) Checking token and check if it's there
  if (
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forgot your password? Submit a PATCH request with your new password and confirmPassword to ${resetUrl}.\n If you didn't forget ypu password please ignore this email. `;
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) If the token is not expired, and their is user set the new password
  if (!user) {
    return next('The token is invalid or has expired', 400);
  }
